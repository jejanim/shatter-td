import { LogSink, LogEvent, LogEventType, LogLevel } from '../Serilog'

export class StringSink implements LogSink {
  ...
  Log(level: LogLevel, events: LogEvent[]): void {
    ...
    for (let index = 0; index < events.length; index++) {
      ...
      if (event.Type == LogEventType.Text) {
        ...
        if (color) message += '|cff' + color
        if (StringSink.Brackets[whichType]) message += '{ '
        message += event.Value
        if (StringSink.Brackets[whichType]) message += ' }'
        if (color) message += '|r'
      }
